...
// ===========================================================================
//	LGAGroupBoxImp.h			PowerPlant 2.1.1	©1997-1999 Metrowerks Inc.
// ===========================================================================
...
void
LGAGroupBoxImp::EnableSelf ()
{
												// Get things redrawn so that we can see the state
												// change
...
void
LGAGroupBoxImp::RefreshBoxBorder ()
{
												// We are only going to worry about doing a refresh
												// if the box is currently visible and it has a
												// frame and a super view
	Rect	theFrame;
	if ( mControlPane->FocusDraw () && IsVisible ()  && 
					mControlPane->CalcPortFrameRect ( theFrame )  && 
						(mControlPane->GetSuperView () != nil)) {

		Rect	revealed;
		LView*	super = mControlPane->GetSuperView ();
		super->GetRevealedRect ( revealed );
		if ( ::SectRect ( &theFrame, &revealed, &revealed )) {
												// Get the content region figured out so that we
												// can use it for the inval
			StRegion		borderRgn ( GetBoxBorderRegion ( revealed ), false );
			
												// Get the content invalidated so that it will be
												// refreshed
			mControlPane->InvalPortRgn(borderRgn);
//			::InvalRgn ( borderRgn );
		}
	}
}
...
void
LGAGroupBoxImp::RefreshBoxContent ()
{
												// We are only going to worry about doing a refresh
												// if the box is currently visible and it has a
												// frame and a super view
	...
	if ( mControlPane->FocusDraw () && IsVisible ()  && 
		...
		if ( ::SectRect ( &theFrame, &revealed, &revealed )) {	
												// Get the content region figured out so that we
												// can use it for the inval
			StRegion		contentRgn ( GetBoxContentRegion ( revealed ), false );
			
												// Get the content invalidated so that it will be
												// refreshed
			mControlPane->InvalPortRgn(contentRgn);
//			::InvalRgn ( contentRgn );
		}
	}
}
...
//------------------------------------------------------------------------------------
//			¥	RefreshBoxHeader																[protected]
//------------------------------------------------------------------------------------
//

void
LGAGroupBoxImp::RefreshBoxHeader ()
{
												// To refresh the header we simply call the object
												// to refresh itself that is if there is one
	if ( IsVisible ()  && HasHeader ())  {
		mHeaderObject->Refresh ();
	}
}


void
LGAGroupBoxImp::DrawWhole ()			// PP CHANGE ADDED
{
	mControlPane->Draw(nil);
	if (mHeaderObject != nil) {
		mHeaderObject->Draw(nil);
	}
}
