...
//-------------------------------------------------------------------------------------
// LGAIconSuiteControl::LGAIconSuiteControl					---<<е CONSTRUCTOR е>>---
//-------------------------------------------------------------------------------------
//
//	Default constructor
										...
										: LControl ( inStream )
{
	...
	*inStream >> mClickInIcon;
	...
	// е If there is no control mode then we need to make sure that
	...
	// е Get the hiliting correctly setup to match the state passed in
	mHilited = GetValue () != 0;	
...
//-------------------------------------------------------------------------------------
// LGAIconSuiteControl::~LGAIconSuiteControl				---<<е DESTRUCTOR е>>---
//-------------------------------------------------------------------------------------
...
{
	// е As a safety measure we will always first make sure that the
...
LGAIconSuiteControl::SetHiliteState	(	Boolean	inHiliteState )
{
	...
	if ( mHilited != inHiliteState )
	{
		...
		// е Things have changed so we need to make sure that the
...
LGAIconSuiteControl::DisableSelf ()
{
	...
	if ( mEnabled == triState_Off || mEnabled == triState_Latent )
	{
		...
		// е Add the disabled transform to the current one this will
