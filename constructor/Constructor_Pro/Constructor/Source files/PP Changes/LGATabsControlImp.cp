...
// ===========================================================================
//	LGATabsControlImp.cp		PowerPlant 2.1.1	й1996-2000 Metrowerks Inc.
// ===========================================================================
...
//-------------------------------------------------------------------------------------
//			е	ApplyForeAndBackColors															[public]
//-------------------------------------------------------------------------------------

bool
LGATabsControlImp::ApplyForeAndBackColors	() const
{
...
void
LGATabsControlImp::ActivateSelf ()
{
												// We have to use refresh here even though it
												// causes a lot more flickering because an
												// activate happens when the window the panel
												// is displayed in is first opened which causes
												// the panel to be drawn twice if we use a call to
												// Draw, this minimizes that at the expense of
												// clean drawing later on in the game.
	if ( IsActive ()) {
		Refresh ();
	}
}
...
void
LGATabsControlImp::DeactivateSelf ()
{
												// We have to use refresh here even though it
												// causes a lot more flickering because an
												// activate happens when the window the panel
...
void
LGATabsControlImp::EnableSelf ()
{
												// Redraw the panel so that the state change takes
												// effect
	if ( IsEnabled ()) {
		Refresh ();
	}
...
void
LGATabsControlImp::DrawWhole ()			// CONSTRUCTOR CHANGE ADDED
{
	mControlPane->Draw(nil);
	if (mTabButtons != nil) {
		mTabButtons->Draw(nil);
	}
}
