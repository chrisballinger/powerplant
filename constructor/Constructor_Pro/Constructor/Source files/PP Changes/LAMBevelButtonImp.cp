...
// ===========================================================================
//	LAMBevelButtonImp.cp		PowerPlant 2.2		©1997-2000 Metrowerks Inc.
// ===========================================================================
...
#else
			// WB1-29485
			// It's a valid case where pictH is NULL.  When the user first
			// chooses the contents to be a PICT resource, usually the
			// resource id field is still set to 0.  When the user changes
			// it to a valid resource id, then everything will work as normal.
			// So if we get a valid picture handle then we will use it but we
			// won't throw an exception if the picture handle is NULL.
			if (pictH != NULL)
			{
				::DetachResource((Handle) pictH);
